...
$data = $db->select('khach_hang','*',$condition,$limit);
...
foreach ($data as $key => $row) {
  ...
  $records["aaData"][] = array(
      ...
      $row['diachi'],
      $row['email'],
      '<a href="home.php?page=customer_action&action=edit&id='.$row['id'].'" class="btn btn-xs default btn-editable"><i class="fa fa-pencil"></i>Sửa</a>
      <a onclick="return confirm(\'Bạn chắc chắn xóa?\')" href="home.php?page=customer_action&action=del&id='.$row['id'].'" class="btn btn-xs default btn-editable"><i class="fa fa-times"></i>Xóa</a>',
    );
}
